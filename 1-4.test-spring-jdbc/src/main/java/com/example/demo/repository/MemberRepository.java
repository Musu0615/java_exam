...
public class MemberRepository {
	...
	public Member save(Member member) {
		...
		if (member.getId() == null) { // ←正しい条件に修正する
			String insertSql = "INSERT INTO test_members(name, age, dep_id) VALUES(:name, :age, :dep_id);";

			// ここに実行処理を書く
			template.update(insertSql, param);
		} else {
			String updateSql = "UPDATE test_members SET name=:name, age=:age, dep_id=:dep_id WHERE id=:id;";
